...
struct url_mhash * url_mhash_init_core(size_t basic_size, size_t static_box_size) {
	size_t len = 0;
	struct url_mhash * mh = (struct url_mhash *)malloc(sizeof(struct url_mhash));
	//
	...
	///////////////////////////////////////
	len = sizeof(struct url_mhash_node) * mh->static_box_size;
	struct url_mhash_node * node = (struct url_mhash_node *)malloc(len);
	//
	memset(node, 0, len);
	//
	mh->head = node;
...
//init
struct url_mhash * url_mhash_init(size_t init_size) {
	return url_mhash_init_core(init_size, init_size);
}

//close
void url_mhash_free(struct url_mhash * mh) {
	struct url_mhash_node * node = 0;
	struct url_mhash_node * node_tmp = 0;
	size_t i = 0;
	//
	if(mh == NULL) {
		return;
...
struct url_mhash_node * url_mhash_seek(struct url_mhash * mh, char * key) {
	size_t idx = url_mhash_index(mh, key);
	struct url_mhash_node * node = mh->head + idx;
	//
	while(node != NULL) {
		if(node->key == NULL) {
			node = NULL;
...
//put
struct url_mhash_node * url_mhash_put_core(struct url_mhash * mh, char * key, void * value, int free_mode) {
	size_t idx = url_mhash_index(mh, key);
	struct url_mhash_node * node = mh->head + idx;
	//
	while(node != NULL) {
	...
	if(mh->static_new_head < mh->head + mh->static_box_size) {
		...
		node->next = (struct url_mhash_node *)malloc(sizeof(struct url_mhash_node));
		mh->box_size += 1;
	}
	//
	node = node->next;
...
struct url_mhash_node * url_mhash_put(struct url_mhash * mh, char * key, void * value) {
	return url_mhash_put_core(mh, key, value, FREE_MODE_NO);
}

//put
struct url_mhash_node * url_mhash_put_clear(struct url_mhash * mh, char * key, void * value) {
	return url_mhash_put_core(mh, key, value, FREE_MODE_YES);
}

//get
void * url_mhash_get(struct url_mhash * mh, char * key) {
	struct url_mhash_node * node = url_mhash_seek(mh, key);
	void * value = 0;
	if(node != NULL) {
		value = node->value;
	}
...
struct url_mhash * url_mhash_clone(struct url_mhash * mh, void * (* value_clone)(char *, void *)) {
	size_t i = 0;
	struct url_mhash * mh_new;
	struct url_mhash_node * node;
	char * key;
	void * value;
	//
	mh_new = url_mhash_init_core(mh->basic_size, mh->box_size);
	//
	for(i = 0; i < mh->basic_size; i ++) {
		node = mh->head + i;
		while(node != NULL) {
			if(node->key != NULL) {
				...
				strcpy(key, node->key);
				//
				url_mhash_put(mh_new, key, value);
			}
			node = node->next;
		}
	}
...
//copy
struct url_mhash * url_mhash_copy(struct url_mhash * mh) {
	size_t i = 0;
	struct url_mhash * mh_new;
	struct url_mhash_node * node;
	//
	...
	for(i = 0; i < mh->basic_size; i ++) {
		...
		while(node != NULL) {
			if(node->key != NULL) {
				url_mhash_put(mh_new, node->key, node->value);
			}
			node = node->next;
		}
	}
...
struct url_mhash_node * url_mhash_to_array(struct url_mhash * mh, size_t * count) {
	struct url_mhash_node * ma;
	size_t i = 0;
	size_t k = 0;
	struct url_mhash_node * node;
	//
	ma = (struct url_mhash_node *)malloc(sizeof(struct url_mhash_node) * mh->data_size);
	//
	for(i = 0; i < mh->basic_size; i ++) {
		node = mh->head + i;
		while(node != NULL) {
			if(node->key != NULL) {
				memmove(ma + k, node, sizeof(struct url_mhash_node));
				k ++;
			}
			node = node->next;
		}
...
int main(int argc, char * argv[]) {
	...
	vardo(0, "diff: %zu, box_size: %zu", ((char *)(mh->static_new_head) - (char *)(mh->head)), sizeof(struct url_mhash_node));
	while(fgets(buf, 1024, fp)) {
		buf_new = (char *)malloc(strlen(buf) + 1);
		memcpy(buf_new, buf, strlen(buf) + 1);
		url_mhash_put(mh, buf_new, buf_new);
	}
	//
	fclose(fp);
	vardo(0, "basic_size: %d, data_size: %d, box_size: %d, static_box_size: %d, head: %zu, new_head: %zu",
		mh->basic_size, mh->data_size, mh->box_size, mh->static_box_size, mh->head, mh->static_new_head);
	vardo(0, "diff: %zu, box_size: %zu", ((char *)(mh->static_new_head) - (char *)(mh->head)), sizeof(struct url_mhash_node));
	mh = url_mhash_copy(mh);
	vardo(0, "basic_size: %d, data_size: %d, box_size: %d, static_box_size: %d, head: %zu, new_head: %zu",
		mh->basic_size, mh->data_size, mh->box_size, mh->static_box_size, mh->head, mh->static_new_head);
	vardo(0, "diff: %zu, box_size: %zu", ((char *)(mh->static_new_head) - (char *)(mh->head)), sizeof(struct url_mhash_node));
	fp = fopen(argv[2], "r");
	while(fgets(buf, 1024, fp)) {
		buf_new = url_mhash_get(mh, buf);
		printf("%s", buf_new);
	}
	fclose(fp);
	//
